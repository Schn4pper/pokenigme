...
export default class AudioPanel {
	...
	private toggleSon(hasAudio: boolean, chargement: boolean = false, langue: Langue): void {
		this._hasAudio = hasAudio;
		if (!hasAudio) {
			this._iconeAudio.innerHTML = '<use href="#icone-son-desactive" fill="var(--couleur-icone)"></use>';
			this._configAudioBouton.setAttribute("aria-label", i18n[langue].audioPanel.activer);
		} else {
			this._iconeAudio.innerHTML = '<use href="#icone-son-active" fill="var(--couleur-icone)"></use>';
			this._configAudioBouton.setAttribute("aria-label", i18n[langue].audioPanel.desactiver);
